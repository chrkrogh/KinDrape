function [Node,P] = KinDrape_eff_NR(d,Grid,Org,Ang,OrgNode,PreShear,Plt) 
...
for i = 1:4
    ...
    for j = GenStart(i,:)' + (0:nGenCell(i)-1).*Dir1(i,:)'
        ...
        P(CellNo,1:4,1:4) = [Node(Idx) abs(Shear)']; 
    end
end
...
for i = 1:4
    for j = ConStart(i,1) + (0:nConCell(i,1)-1)*(Dir1(i,1)+Dir2(i,1))
        for k = ConStart(i,2) + (0:nConCell(i,2)-1)*(Dir1(i,2)+Dir2(i,2))
            % Get cell idx and no. Call MoldCircIntersecFun to get Vert #3
